...
(define-struct pipe-info (x gap-y))
(define-struct variables (gravity momentum scroll-speed))

(define (pipes-list)
  (list (pipe-info scene-width
...
(define (pipe->image p)
  (above (rectangle pipe-width (pipe-info-gap-y p) "solid" "green")
         (rectangle pipe-width pipe-self-gap "solid" "white")
         (rectangle pipe-width (max 0 (- scene-height pipe-self-gap (pipe-info-gap-y p))) "solid" "green")))
...
(define-struct abilities-pair (visible active))

(define initial-abilities-pair (abilities-pair '() '()))
(define initial-variables (variables initial-gravity
                                     initial-momentum
                                     initial-scroll-speed))

(define initial-score 0)
(define (get-initial-state)
  (state (bird-state bird-x bird-initial-y 0)
         (pipes-list)
         initial-score
         initial-abilities-pair
         initial-variables))
;TODO 2
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-state _ y v-y) bird])
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy state current-state
                   [bird (next-state-bird-onspace (state-bird current-state)
                                                  initial-momentum)])
      current-state))
...
; sub formă de lista.
(define (get-pipes current-state)
  (state-pipes current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (p)
         (struct-copy pipe-info p [x (- (pipe-info-x p) scroll-speed)]))
       pipes))
...
(define (add-more-pipes pipes)
  (if (= (length pipes) no-pipes)
      ...
      (append pipes
            (list (pipe-info
                       (+ pipe-width pipe-gap (pipe-info-x (last pipes)))
                       (+ added-number (random random-threshold)))))))
...
(define (get-score current-state)
  (state-score current-state))
...
(define (check-ground-collision bird)
  (>= (+ (bird-state-y bird) bird-height) ground-y))
...
(define (invalid-state? state)
  (or (check-ground-collision (state-bird state))
      (check-pipe-collisions (state-bird state) (state-pipes state))))
...
(define (check-pipe-collisions bird pipes)
  (ormap (λ (p)
           (let ([bird-topleft (make-posn bird-x (bird-state-y bird))]
                 [bird-bottomright (make-posn (+ bird-x bird-width)
                                              (+ (bird-state-y bird) bird-height))])
             (or (check-collision-rectangles
                  bird-topleft bird-bottomright
                  (make-posn (pipe-info-x p) 0)
                  (make-posn (+ (pipe-info-x p) pipe-width) (pipe-info-gap-y p)))
                 (check-collision-rectangles
                  bird-topleft bird-bottomright
                  (make-posn (pipe-info-x p) (+ (pipe-info-gap-y p) pipe-self-gap))
                  (make-posn (+ (pipe-info-x p) pipe-width) pipe-height))
                 )))
         pipes))
...
(define (next-state curr)
  (struct-copy state curr
               [bird (next-state-bird (state-bird curr)
                                      (variables-gravity (state-variables curr)))]
               [pipes (next-state-pipes (state-pipes curr)
                                        (variables-scroll-speed (state-variables curr)))]
               [score (+ 0.1 (state-score curr))]
               [abilities-pair (next-abilities (state-abilities-pair curr)
                                               (state-bird curr)
                                               (variables-scroll-speed (state-variables curr)))]
               [variables (next-variables initial-variables (state-abilities-pair curr))]))
...
(define (draw-world state)
  (place-image/align
   ...
   0 ground-y "left" "top"
   (place-image
    (score-to-image (state-score state))
    text-x text-y
    (place-active-abilities
     (state-abilities-pair state)
     (place-visible-abilities
      (state-abilities-pair state)
      (place-pipes (state-pipes state) initial-scene))))))

(define (draw-frame state)
  (place-image/align bird-image
                     (+ -0.5 bird-x) (+ -0.5 (bird-state-y (state-bird state)))
                     "left" "top"
                     (draw-world state)))
...
(define (place-pipes pipes scene)
  (place-images/align
   (map pipe->image pipes)
   (map (λ (p) (make-posn (pipe-info-x p) 0)) pipes)
   "left"
   "top"
   scene))
...
(define fast-ability (ability (hourglass "tomato") 30 null (λ (x)
                                                             (struct-copy variables x
                                                                          [scroll-speed (+ (variables-scroll-speed x) 1)]))))
...
(define slow-ability (ability (hourglass "mediumseagreen") 10 null
                              (λ (x)
                                 (struct-copy variables x
                                         [scroll-speed (max 5 (- (variables-scroll-speed x) 1))]))))
...
(define (clean-abilities abilities)
  (filter (λ (a)
            (> (posn-x (get-ability-pos a)) 0))
          abilities))


(define (posn-subtract a b)
...
(define (move-abilities abilities scroll-speed)
  (map (λ (a)
         (struct-copy ability a [pos (posn-subtract (get-ability-pos a)
                                                    (make-posn scroll-speed 0))]))
       abilities))
...
(define (time-counter abilities)
  (filter (λ (a)
            (> (ability-time a) 0))
          (map (λ (a)
                 (struct-copy ability a
                              [time (- (ability-time a) (/ 1 fps))]))
               abilities)))
...
(define (ability-collision ability bird)
   (let* ([bird-topleft (make-posn bird-x (bird-state-y bird))]
         [bird-bottomright (make-posn (+ bird-x bird-width)
                                      (+ (bird-state-y bird) bird-height))]
         [ability-img (ability-image ability)]
         [ability-pos (ability-pos ability)]
         [pos-x-left (- (posn-x ability-pos) (quotient (image-width ability-img) 2))]
         [pos-x-right (+ (posn-x ability-pos) (quotient (image-width ability-img) 2))]
         [pos-y-up (- (posn-y ability-pos) (quotient (image-width ability-img) 2))]
         [pos-y-down (+ (posn-y ability-pos) (quotient (image-width ability-img) 2))])
     (check-collision-rectangles
      bird-topleft bird-bottomright
      (make-posn pos-x-left pos-y-up)
      (make-posn pos-x-right pos-y-down))))

(define (abilities-collision abilities bird)
  (filter (λ (a)
            (ability-collision a bird))
          (get-abilities-visible abilities)))

(define (next-abilities abilities bird scroll-speed)                       
  (struct-copy abilities-pair abilities
               [active (time-counter (append (abilities-pair-active abilities) (abilities-collision abilities bird)))]
               [visible (next-abilities-visible
                         (remove* (abilities-collision abilities bird) (abilities-pair-visible abilities))
                         scroll-speed)]))
...
(define (place-visible-abilities abilities scene)
  (place-images
   (map ability-image (get-abilities-visible abilities))
   (map ability-pos (get-abilities-visible abilities))
   scene))
...
(define (index-list abilities x init)
  (if (= (length abilities) 0)
      x
      (index-list (list-tail abilities 1) (cons init x) (+ init 1)))) 

(define (place-active-abilities abilities scene)
  (let* ([imgs (map ability-image (get-abilities-active abilities))]
         [scale (map (λ (img) (scale 0.75 img)) imgs)])
    (place-images
     scale
     (map (λ (p)
            (make-posn (- (posn-x abilities-posn) (* 50 p)) (posn-y abilities-posn)))
          (index-list (get-abilities-active abilities) '() 0))
     scene)))
